...
local showtabline_value = vim.api.nvim_get_option('showtabline')

function update_tab_mapping()
  local showtabline_value = vim.api.nvim_get_option('showtabline')
  wk.register({
    ["<Tab>"] = {
      name = "workspace",
      ["<Tab>"] = {
        string.format(":set showtabline=%d<CR>", showtabline_value == 2 and 0 or 2),
    ...
    g = {
      ...
      g = { ":Neogit kind=vsplit<CR>", "Neogit status" },
...
vim.api.nvim_exec([[
  augroup MyTabMappingUpdate
    autocmd!
    autocmd OptionSet showtabline lua update_tab_mapping()
  augroup END
]], false)
